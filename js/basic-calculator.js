...
let errorMessage = document.getElementById('error-card');
...
_sqroot.addEventListener('click', ()=>{
	...
	if(aValue == '' || aValue == 0){
		...
		errorMessage.style.display = 'none'; // Hide Message Error
...
_square.addEventListener('click', ()=>{
	...
	if(aValue == '' || aValue == 0){
		...
		const result = Math.pow(parseFloat(aValue), 2);
		currentDisplay.value = result.toFixed(result % 1 === 0 ? 0 : 10).replace(/\.?0+$/, '');
	}
});
...
_reciprocal.addEventListener('click', () => {
    ...
    if (aValue === '' || aValue == 0) {
		var textErrorMessage = document.getElementById('error-text');
        textErrorMessage.textContent = 'Error:  No se puede obtener el recíproco de 0.';
        errorMessage.style.display = 'block';
        currentDisplay.value = 0; 

    } else {
        errorMessage.style.display = 'none'; // Hide Message Error
        currentDisplay.value = 1 / aValue;
    }
});
...
let refreshHistoryTab = function () {
	...
	if (!_anyPills.length) {
		document.querySelector('#historyTab').innerHTML = '<span id="emptyPill">Todavía no hay historial</span>';
	} else {
...
clearHistoryBtn.addEventListener('click', function() {
	document.querySelector('#historyTab').innerHTML = '<span id="emptyPill">Es historial está vacío.</span>';
});
...
if (_toggleDLMode) {
	_toggleDLMode.addEventListener('click',function(){
		...
		if (_htmlTag.classList.contains('calc-darkmode')) {
			...
			_toggleDLMode.innerHTML = '<i class="fa fa-sun"></i>&nbsp;Modo Claro';
			...
			_toggleDLMode.innerHTML = '<i class="fa fa-moon"></i>&nbsp;Modo Oscuro';
